...
function ItemDAO(database) {
    ...
    this.searchItems = function (query, page, itemsPerPage, callback) {
        ...
        var queryCondition = { "$text": {"$search": query} };
    if (query.trim() == "") queryCondition = {};
    var cursor = this.db.collection("item").find(queryCondition);
    cursor.skip(page*itemsPerPage);
    cursor.limit(itemsPerPage);
    cursor.toArray(function(err, pageItems) {
        if(err) callback(err);
    ...
    this.getNumSearchItems = function (query, callback) {
        ...
        var queryCondition = { "$text": {"$search": query} };
    if (query.trim() == "") queryCondition = {};
    this.db.collection("item").find(queryCondition).count(function(err, count) {
        if(err) callback(err);
        callback(count);
    });
    }
